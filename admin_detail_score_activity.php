...
// ==========================================
// 2. จัดการการบันทึกข้อมูล (เมื่อกดปุ่มบันทึกคะแนน)
// ==========================================
...
// ==========================================
// 4. ดึงรายชื่อผู้เข้าร่วม (เฉพาะคนที่ approved)
// ==========================================
$participants = [];
$reg_ids = []; 
...
// ==========================================
// 5. ดึงข้อมูล "หลักฐานการเข้าร่วม" จาก activity_evidences
// ==========================================
$evidences = [];
if (!empty($reg_ids)) {
...
<head>
    ...
    .navbar { padding: 20px; }
    .main-content { margin: 30px 50px; padding: 20px; }
    .card-header-custom { background-color: #fff; border-bottom: 2px solid #e3e6f0; padding: 1.5rem; }
    .table th { background-color: #f8f9fc; color: #4e73df; }
    .radio-custom { transform: scale(1.2); margin-right: 5px; cursor: pointer; }
    .evidence-img { max-width: 100%; height: auto; border-radius: 10px; margin-bottom: 10px; border: 1px solid #ddd; }
    ...
    <div class="main-content">
        <div class="container-fluid">
            ...
            <div class="mb-4">
                ...
                <h3 class="fw-bold mb-0 text-gray-800"><i class="fa-regular fa-star text-primary me-2"></i> ให้คะแนน: <?php echo htmlspecialchars($activity_title); ?></h3>
            </div>

            <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                <div class="card-header card-header-custom d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 fw-bold">รายชื่อผู้เข้าร่วมที่ได้รับการอนุมัติ</h5>
                    <span class="badge bg-primary rounded-pill px-3 py-2">ทั้งหมด <?php echo count($participants); ?> คน</span>
                </div>
                    ...
                    <?php if (count($participants) > 0): ?>
                        <form method="POST" action="admin_detail_score_activity.php?id=<?php echo htmlspecialchars($current_activity_id); ?>">
                                ...
                                <table class="table table-hover align-middle mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><span class="badge bg-light text-dark border"><?php echo htmlspecialchars($user['student_id']); ?></span></td>
                                            <td class="fw-bold"><?php echo htmlspecialchars($user['first_name'] . ' ' . $user['last_name']); ?></td>
                                            <td><?php echo !empty($user['task_name']) ? htmlspecialchars($user['task_name']) : '<span class="text-muted">-</span>'; ?></td>
                                            
                                            <td class="text-center">
                                                <?php if ($has_evidence): ?>
                                                    <button type="button" class="btn btn-sm btn-outline-info rounded-pill" data-bs-toggle="modal" data-bs-target="#evidenceModal_<?php echo $reg_id; ?>">
                                            ...
                                            <td class="text-center bg-light">
                                                <div class="d-flex justify-content-center gap-3">
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input radio-custom border-warning" type="radio" name="scores[<?php echo $reg_id; ?>]" id="wait_<?php echo $reg_id; ?>" value="waiting" <?php echo ($current_status == 'waiting' || empty($current_status)) ? 'checked' : ''; ?>>
                                                        <label class="form-check-label text-warning fw-bold" for="wait_<?php echo $reg_id; ?>">รอผล</label>
                                                    </div>
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input radio-custom border-success" type="radio" name="scores[<?php echo $reg_id; ?>]" id="pass_<?php echo $reg_id; ?>" value="passed" <?php echo ($current_status == 'passed') ? 'checked' : ''; ?>>
                                                        <label class="form-check-label text-success fw-bold" for="pass_<?php echo $reg_id; ?>"><i class="fa-solid fa-check"></i> ผ่าน</label>
                                                    </div>
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input radio-custom border-danger" type="radio" name="scores[<?php echo $reg_id; ?>]" id="fail_<?php echo $reg_id; ?>" value="not_passed" <?php echo ($current_status == 'not_passed') ? 'checked' : ''; ?>>
                                                        <label class="form-check-label text-danger fw-bold" for="fail_<?php echo $reg_id; ?>"><i class="fa-solid fa-xmark"></i> ไม่ผ่าน</label>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
                            ...
                            <div class="p-4 bg-white border-top text-end">
                                <button type="submit" name="save_scores" class="btn btn-success px-5 py-2 fw-bold" style="border-radius: 10px;">
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="fa-solid fa-file-invoice me-2"></i> หลักฐานของ <?php echo htmlspecialchars($user['first_name'] . ' ' . $user['last_name']); ?></h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body bg-light">
                    <div class="row g-3">
                        <?php foreach ($evidences[$reg_id] as $ev): 
                            // เปลี่ยนจาก file_path เป็น image_path ตามฐานข้อมูลแล้ว
                        ...
                        <div class="col-12">
                            <div class="card border-0 shadow-sm p-3">
                                <?php if (!empty($text)): ?>
                                    <p class="mb-2 border-bottom pb-2"><strong>ข้อความอธิบาย:</strong> <?php echo nl2br(htmlspecialchars($text)); ?></p>
                                <?php endif; ?>
                                
                                <?php if (!empty($file)): ?>
                                    <?php 
                                        // เช็คว่าเป็นรูปภาพ หรือไฟล์แนบประเภทอื่น
                                        $ext = strtolower(pathinfo($file, PATHINFO_EXTENSION));
                                        if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'])): 
                                    ?>
                                        <img src="uploads/evidences/<?php echo htmlspecialchars($file); ?>" class="evidence-img" alt="หลักฐาน">
                                    <?php else: ?>
                                        <a href="uploads/evidences/<?php echo htmlspecialchars($file); ?>" target="_blank" class="btn btn-outline-primary btn-sm mt-2">
